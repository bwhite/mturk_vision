...
class AMTManager(object):
    ...
    def admin_users(self, secret):
        ...
        if secret == self.secret:
            return dict((k.split(':', 1)[1], self.users_db.hgetall(k)) for k in self.users_db.keys(self.prefix + '*') if k.startswith(self.prefix))

    def admin_results(self, secret):
        """Return contents of response_db"""
        if secret == self.secret:
            return dict((k.split(':', 1)[1], self.response_db.hgetall(k)) for k in self.response_db.keys(self.prefix + '*') if k.startswith(self.prefix))
