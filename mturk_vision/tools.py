...
def create_hit(title, description, duration, reward, max_assignments, keywords, url,
               max_spend=10, sandbox=False, mturk=None):
    kw = {}
    if sandbox:
        kw = {'host': 'mechanicalturk.sandbox.amazonaws.com'}
    if mturk is None:
        mturk = boto.connect_mturk(os.environ['AWS_ACCESS_KEY_ID'], os.environ['AWS_SECRET_ACCESS_KEY'], **kw)
    ...
    return mturk.create_hit(question=question,
